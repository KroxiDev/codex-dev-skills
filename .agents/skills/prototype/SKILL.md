...
- **"¿Esta lógica / modelo de estado se siente correcto?"** → [logica.md](references/logica.md). Construir una pequeña app interactiva de terminal que empuje la máquina de estados por casos difíciles de razonar sobre el papel.
...
2. **Un comando para ejecutarlo.** Lo que el task runner existente del proyecto soporte — `pnpm <nombre>`, `python <ruta>`, `bun <ruta>`, etc. El usuario debe poder arrancarlo sin pensar.
